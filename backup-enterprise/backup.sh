...
WORK_DIR="$(mktemp -d)"
PLAIN_ARCHIVE="$WORK_DIR/mail-control-enterprise-${TIMESTAMP}.tar.gz"
ARCHIVE="/backups/mail-control-enterprise-${TIMESTAMP}.tar.gz.enc"
...
on_exit() {
    ...
    rm -rf "$WORK_DIR"
    if (( status != 0 )); then
        notify_telegram "🔴 <b>BACKUP ENTERPRISE FALLIDO</b>

Sistema: <b>Mail Control Enterprise</b>
...
fi
{
    ...
    printf 'database=%s\n' "$POSTGRES_DB"
    printf 'system=mail-control-enterprise\n'
...
if [[ "$(date -u +%d)" == "01" ]]; then
    MONTHLY_NAME="mail-control-enterprise-monthly-$(date -u +%Y%m).tar.gz.enc"
...
while IFS= read -r remote_file; do
    ...
    if [[ "$filename" =~ ^mail-control-enterprise-([0-9]{8})-([0-9]{6})\.tar\.gz\.enc$ ]]; then
        file_timestamp="${BASH_REMATCH[1]}${BASH_REMATCH[2]}"
        if [[ "$file_timestamp" < "$CUTOFF" ]]; then
            mega-rm "$remote_file"
        fi
    fi
done < <(mega-find "$DAILY_FOLDER" --type=f --pattern='mail-control-enterprise-*.tar.gz.enc')

MONTH_CUTOFF="$(date -u -d "-${KEEP_MONTHS} months" +%Y%m)"
while IFS= read -r remote_file; do
    filename="$(basename "$remote_file")"
    if [[ "$filename" =~ ^mail-control-enterprise-monthly-([0-9]{6})\.tar\.gz\.enc$ ]]; then
        if [[ "${BASH_REMATCH[1]}" < "$MONTH_CUTOFF" ]]; then
            mega-rm "$remote_file"
        fi
    fi
done < <(mega-find "$MONTHLY_FOLDER" --type=f --pattern='mail-control-enterprise-monthly-*.tar.gz.enc')

find /backups -type f -name 'mail-control-enterprise-*.tar.gz.enc' -mtime +1 -delete
if [[ "${BACKUP_NOTIFY_SUCCESS:-true}" == "true" ]]; then
    notify_telegram "✅ <b>BACKUP ENTERPRISE COMPLETADO</b>

Sistema: <b>Mail Control Enterprise</b>
Archivo: <code>$(basename "$ARCHIVE")</code>
Retención: ${KEEP_DAYS} días + ${KEEP_MONTHS} meses."
fi
echo "Backup Enterprise cifrado completado: $(basename "$ARCHIVE")"
